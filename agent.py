...
DASHBOARD_URL       = "http://localhost:5050/event"
...
def emit_event(event_type, **kwargs):
    """Send real-time event to dashboard. Silently fails if not running."""
    try:
...
def speak(text, lang=None):
    ...
    emit_event("speaking", text=text, lang=active_lang)
...
def read_screen_content():
    """Reads screen content. If WhatsApp chat open — reads last 4 messages naturally."""
    _, b64 = capture_screen()
    prompt = (
...
def normalize_command(command):
    """Uses Gemma 4 to translate any language command to English."""
...
def find_element_via_uia(target_name):
    try:
        ...
        matches = difflib.get_close_matches(
            target_name.lower(), [n.lower() for n in names], n=1, cutoff=0.8)
...
def smart_click(target):
    emit_event("layer", layer="uia", status="active")
    result = find_element_via_uia(target)
    if result:
        cx, cy = result
        emit_event("layer", layer="uia", status="success")
        print(f"[L1a-UIA] clicking ({cx}, {cy})")
        pyautogui.moveTo(cx, cy, duration=0.3)
        time.sleep(0.2)
        pyautogui.click(cx, cy)
        return True
    emit_event("layer", layer="uia", status="failed")

    emit_event("layer", layer="cdp", status="active")
    result = find_element_via_cdp(target)
    if result:
        cx, cy = result
        emit_event("layer", layer="cdp", status="success")
        print(f"[L1b-CDP] clicking ({cx}, {cy})")
        pyautogui.moveTo(cx, cy, duration=0.3)
        time.sleep(0.2)
        pyautogui.click(cx, cy)
        return True
    emit_event("layer", layer="cdp", status="failed")

    TAB_KEYWORDS = ["tab", "untitled", "jupyter", ".ipynb", ".py",
                    ".js", ".html", ".css", ".txt", ".md", "home"]
    if any(kw in target.lower() for kw in TAB_KEYWORDS):
        emit_event("layer", layer="tab", status="active")
        print("[L2-TAB] using tab strip click")
        success = click_tab_by_name(target)
        emit_event("layer", layer="tab", status="success" if success else "failed")
        return success

    emit_event("layer", layer="vision", status="active")
    ...
    if coords:
        ...
        emit_event("layer", layer="vision", status="success")
        print(f"[L3] clicking ({click_x}, {click_y})")
        pyautogui.moveTo(click_x, click_y, duration=0.4)
        time.sleep(0.2)
        pyautogui.click(click_x, click_y)
        return True
    emit_event("layer", layer="vision", status="failed")
    return False
...
def check_urgent_emails():
    ...
    session["last_emails"] = emails
    email_text = "\n".join(
        f"From {e['sender']}: {e['subject']} — {e['preview']}" for e in emails)
    prompt = (
...
def find_target_email(raw_command):
    """Find which email user wants to reply to by number or sender name."""
    ...
    for word, idx in number_words.items():
        if word in c and idx < len(emails):
            return emails[idx]

    for email in emails:
        sender = email.get("sender", "").lower()
        sender_first = sender.split()[0] if sender else ""
        if sender_first and len(sender_first) > 2:
            if sender_first in c or sender.split("@")[0] in c:
                return email

    return emails[0]


def reply_to_last_email(reply_text, recognizer, mic, raw_command=""):
    emails = session.get("last_emails", [])
    if not emails:
        return "Please read your emails first so I know which one to reply to."

    target_email = find_target_email(raw_command) or emails[0]
    speak(f"Replying to email from {target_email['sender']}.")
    ...
    try:
        ...
        sender_safe  = target_email["sender"].replace("'", "\\'")[:30].lower()
        subject_safe = target_email["subject"].replace("'", "\\'")[:30].lower()
        ...
        (function() {{
            ...
            for (const row of rows) {{
                ...
                const subjEl   = row.querySelector('.bog span, .bog');
                const senderTxt = (senderEl?.innerText || '').toLowerCase();
                const subjTxt   = (subjEl?.innerText || '').toLowerCase();
                if (senderTxt.includes('{sender_safe}') || subjTxt.includes('{subject_safe}')) {{
        ...
        speak(f"Reply typed: {reply_text}. Say yes to send or no to cancel.")
        confirmation = listen_for_command(recognizer, mic, timeout=8)
        print(f"[GMAIL REPLY] Confirmation: '{confirmation}'")

        if any(w in confirmation for w in ["yes", "send", "okay", "confirm", "do it", "go ahead"]):
            pyautogui.hotkey("ctrl", "enter")
            return f"Reply sent to {target_email['sender']} successfully."
        else:
            pyautogui.hotkey("ctrl", "shift", "d")
            return "Reply cancelled. Email was not sent."
...
KEYWORD_RULES = [
    ...
    (["reply to the last email", "reply to last email",
      ...
      "reply to email", "reply to the email from", "reply to"],
     lambda c: {"action": "reply_email", "raw": c, "text": ""}),

    # General questions → chat handles everything
...
def start_accessibility_loop():
    ...
    while True:
        ...
        print(f"\n🎙️  Heard: '{command}'")
        emit_event("heard", command=command)
        ...
        method = "keyword" if keyword_parse(command_en) else "gemma"
        params = {k: v for k, v in parsed.items() if k != "action"}
        emit_event("intent", action=action, params=params, method=method)
        ...
        if action == "open_app":
            ...
            ALIAS_MAP = {
                ...
                "good hackathon":     "gemma 4 good hackathon",
                "gemma hackathon":    "gemma 4 good hackathon",
                "gemma 4 hackathon":  "gemma 4 good hackathon",
                "the hackathon":      "gemma 4 good hackathon",
                "kaggle hackathon":   "gemma 4 good hackathon",
                "good health":        "gemma 4 good hackathon",
                "good headphone":     "gemma 4 good hackathon",
                "good helper":        "gemma 4 good hackathon",
                "join hackathon":     "join competition",
                "joint hackathon":    "join competition",
                "joint account":      "join competition",
                ...
                try:
                    ...
                    match = next(
                        (p for p in pages
                         ...
                         or target.lower() in p.get("title", "").lower()), None)
                    if match:
            ...
            if any(query.endswith(tld) for tld in [".com", ".org", ".net", ".io", ".in"]):
                ...
                results = call_ollama(
                    "List the titles of the top 3 search results visible on screen. One sentence only.",
            ...
            _, b64 = capture_screen(max_size=1920)
            desc = call_ollama_chat("Describe what is on my screen right now in 2-3 sentences.", b64_image=b64)
            speak(desc)
            ...
            raw_command = parsed.get("raw", command)
            if not reply_text:
                speak("What would you like to say in the reply?")
                reply_text = listen_for_command(recognizer, mic, timeout=10)
            if reply_text:
                result = reply_to_last_email(reply_text, recognizer, mic, raw_command=raw_command)
                speak(result)
            ...
            emit_event("language", lang=lang_code)
            speak(f"Language switched to {lang_name}.", lang=lang_code)
            log_event(command, action, lang_name)

        else:
            # chat + unknown both handled here with vision
            _, b64 = capture_screen(max_size=1920)
            answer = call_ollama(
                f"You are a voice assistant for a completely blind user. "
                f"They said: '{command}' "
                f"If this is about the screen, look at the screenshot and answer fully. "
                f"If this is a general question, answer it clearly. "
                f"Respond in natural spoken sentences, 2-3 sentences max. "
                f"Never say you cannot see the screen.",
                b64_image=b64, temperature=0.3
            )
            if not answer:
                answer = "I am not sure about that. Could you rephrase?"
            speak(answer)
            log_event(command, "chat", answer)

        emit_event("responding",
                   text=session["last_spoken"],
                   lang=session["tts_lang"],
                   action=action,
                   ms=int((time.time() - t_start) * 1000))
        emit_event("done", command=command, action=action, success=True)

        # Update conversation context after screen-changing actions
        if action in ("open_app", "navigate", "search", "click_element", "new_tab"):
            time.sleep(1.2)
            _, b64 = capture_screen(max_size=1920)
            ctx = call_ollama(
            ...
            time.sleep(1.0)
            session["last_screen_desc"] = describe_screen()
